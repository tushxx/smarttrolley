...
const STEPS = [
  { n: "01", title: "Place item in trolley", body: "The camera on the handle immediately points at the product you just dropped in." },
  { n: "02", title: "AI detects in seconds", body: "YOLO11s runs on-device on a Raspberry Pi 5. No internet required. Result in under 3 s." },
  { n: "03", title: "Pay and walk out", body: "Your running total is on the trolley screen. Tap pay via Razorpay when you're done." },
];
...
export default function Landing() {
  ...
  const loginMutation = useMutation({
    ...
    onError: () => toast({ title: "Error", description: "Could not start session. Try again.", variant: "destructive" }),
  });
    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <nav className="flex items-center justify-between px-8 py-5 border-b border-gray-100">
        <a href="/" className="flex items-center gap-2.5">
          <Logo />
          <span className="text-[17px] font-semibold text-gray-900 tracking-tight">SmartCart</span>
        </a>
        <span className="text-xs text-gray-400 hidden sm:block">Final Year Project · 2025</span>
      </nav>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-20 text-center">
        ...
        <p className="mt-6 text-lg text-gray-500 max-w-md leading-relaxed">
          An AI camera on your trolley detects every item you place inside.
          Pay from the screen and walk straight out.
        </p>
        ...
        <div className="mt-10 w-full max-w-sm">
          <div className="flex gap-2">
            <div className="relative flex-1">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-sm text-gray-400 select-none pointer-events-none">+91</span>
              <input
                ref={inputRef}
                type="tel"
                value={phone}
          ...
          <p className="mt-3 text-xs text-gray-400 text-center">No OTP · No password · Instant access</p>
        </div>
        ...
        <div className="mt-16 w-full max-w-4xl grid grid-cols-1 sm:grid-cols-3 gap-10 text-left">
          {STEPS.map((s) => (
            <div key={s.n}>
              <span className="text-xs font-semibold text-green-600 tracking-widest">{s.n}</span>
